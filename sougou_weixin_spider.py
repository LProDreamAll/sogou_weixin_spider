...
class WeixinSogou:
	...
	def gen_pv_url(self):
		url_pre = "https://pb.sogou.com/pv.gif?uigs_t={}".format(int(1000*time.time()))
	...
	def fetch(self, furl, url):
		print('crawling...', url)
		html = self.session.get(furl)
		html.encoding = 'utf8'
		self.update_uigs_para(html.text)
		if 'tsn' in url:
	...
	def extract_search_link(self, html):
		hrefs = re.findall(r'<h3>.*?<a .*?href="(.*?)"', html, re.DOTALL)
		hrefs = set(hrefs)
	...
	def extract_weixin_data(self, url, html):
		#with open('z-weixin-post.html', 'w', encoding='utf-8') as f:
		#	f.write(html)
		try:
	...
	def process_weixin(self, sogou_link, i):
		...
		if 'http://mp.weixin.qq.com/profile' in weixin_url:
			print('weixin user url:', weicin_url)
			return None
	...
	def process(self, furl, url):
		html = self.fetch(furl, url)
...
if __name__ == '__main__':
	...
	if opt == 'search':
		cc.init_session()
		furl,url = cc.gen_url('Python', tsn = '1')
		links, nexts = cc.process(furl,url)
		print(links)
		print(nexts)
		for i,link in enumerate(links):
			item = cc.process_weixin(link, i)
			time.sleep(1)
			pprint(item)
